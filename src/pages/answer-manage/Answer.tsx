...
import { getAnswer } from "../../services/pages";
...
const Answer = () => {
  const [dataList, setDataList] = useState<AnswerType[]>([]);
  ...
  const editData = (item: AnswerType) => {
    navigate("/answer/edit", { state: { item } });
  };
    ...
    <div className="dashboard">
      <div className="header-section">
        ...
        <div className="right-section">
          <NavLink className="nav-link" to="/answer/add">
            Add New
          </NavLink>
        </div>
      </div>
            ...
            <table>
              ...
              <tbody>
                {dataList.map((item, index) => (
                  <tr key={item.answer_text}>
                    <td className="text-ceter">{index + 1}</td>
                    <td>{item?.question}</td>
                    <td>{item?.answer_text}</td>
                    <td>{item?.is_correct == "Y" ? "Correct" : "Wrong"}</td>
                    <td className="text-ceter">
                      <div className="icon-wrapper">
                        ...
                        <svg
                          className="cursor-pointer"
                          onClick={() => {
                            handleShow(item.id!, item.answer_text);
                          }}
                          xmlns="http://www.w3.org/2000/svg"
                          viewBox="0 0 448 512"
                          width={15}
